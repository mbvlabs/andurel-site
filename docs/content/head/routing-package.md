...
`InertiaRoute()` marks a route for `andurel sync routes`, which writes typed helpers to `resources/js/routes.ts`. Opt-in avoids exporting internal endpoints or assets into the browser bundle. `JsExpr` marks a JavaScript expression for generator interpolation; it is not a general escaping function.
